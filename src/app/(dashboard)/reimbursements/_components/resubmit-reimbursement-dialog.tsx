...
import { updateReimbursement } from "@/app/actions/reimbursement";
import { Loader2, UploadCloud, RefreshCw } from "lucide-react";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";

export function ResubmitReimbursementDialog({ reimbursement }: { reimbursement: any }) {
    const [isOpen, setIsOpen] = useState(false);
    const [loading, setLoading] = useState(false);
    const [uploading, setUploading] = useState(false);

    const [amount, setAmount] = useState(reimbursement.amount.toString());
    const [description, setDescription] = useState(reimbursement.description);
    const [category, setCategory] = useState(reimbursement.category || "General");
    const [receiptUrl, setReceiptUrl] = useState<string | null>(reimbursement.receiptUrl);
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        const result = await updateReimbursement(reimbursement.id, {
        ...
        if (result.success) {
            toast.success("Reimbursement claim resubmitted successfully");
            setIsOpen(false);
        } else {
            toast.error(result.error || "Failed to resubmit claim");
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Resubmit Reimbursement Claim</DialogTitle>
                    <DialogDescription>
                        Update any incorrect details or add missing receipts, then resubmit for approval.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid gap-4 py-4">
                    {reimbursement.adminNotes && (
                        <div className="p-3 bg-red-50 text-red-800 rounded-md border border-red-200 text-sm">
                            <span className="font-semibold block mb-1">Admin Rejection Note:</span>
                            {reimbursement.adminNotes}
                        </div>
                    )}
                        ...
                        {receiptUrl ? (
                            <div className="flex items-center gap-2 p-2 border rounded bg-slate-50 text-sm">
                                <span className="truncate flex-1 text-blue-600">
                                    <a href={receiptUrl} target="_blank" rel="noreferrer">View Current Receipt</a>
                                </span>
                            ...
                            <div className="relative">
                                ...
                                <Button variant="outline" className="w-full border-dashed">
                                    {uploading ? (
                                        ...
                                        <><UploadCloud className="h-4 w-4 mr-2 text-slate-500" /> Attach New Receipt</>
                                    )}
                                </Button>
                            </div>
                        )}
